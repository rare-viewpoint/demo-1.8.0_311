 ...
 * @Date 2021/12/5 5:13
 */
public class SignatureRSADemo {

    private final static String SIGNATURE_ALGORITHM = "SHA1withRSA";
    private final static String KEY_PAIR_ALGORITHM = "RSA";
...
//       return RSAPrivateKeyImpl.parseKey(derValue);
       return RSAPrivateCrtKeyImpl.parseKey(derValue);
    }

    static PublicKey getPublicKey(String pubKey) throws IOException {
        byte[] bytes = Base64.getDecoder().decode(pubKey);
        DerValue derValue = new DerValue(bytes);
        return RSAPublicKeyImpl.parse(derValue);
    }
...
//  //              signature2.initVerify(getPublicKey(pubKey));
//                signature2.update(line.getBytes());
//                boolean result2 = signature2.verify(bytes);
